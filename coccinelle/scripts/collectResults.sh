...
collect(){
   ...
   comm -23 allwithoutreassignedstrictdependent.tmp $RESULTS/$1ereport/functionnamesonly.out | sort | uniq > allwithoutreassignedstrictdependentereport.tmp
   ...
   grep "^>" allwithoutreassignedstrictdependent.tmp | sort | uniq | wc -l >> $STATS_FILE
