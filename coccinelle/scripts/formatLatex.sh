...
echo "      C1: Strict Functions& $FREE & $REALLOC \\\\
" >> $LATEX_FILE

FREE=$(grep -A 1 "DEPENDENT:" ../results/freestats.out | tail -n 1)
REALLOC=$(grep -A 1 "DEPENDENT:" ../results/reallocstats.out | tail -n 1)
echo "      C2: Dependent Functions& $FREE & $REALLOC \\\\
" >> $LATEX_FILE

FREE=$(grep -A 1 "EREPORT:" ../results/freestats.out | tail -n 1)
REALLOC=$(grep -A 1 "EREPORT:" ../results/reallocstats.out | tail -n 1)
echo "      C3: Error Functions& $FREE & $REALLOC \\\\
" >> $LATEX_FILE

FREE=$(grep -A 1 "SAME RETURN:" ../results/freestats.out | tail -n 1)
REALLOC=$(grep -A 1 "SAME RETURN:" ../results/reallocstats.out | tail -n 1)
echo "      C5: Reassign& $FREE & $REALLOC \\\\
" >> $LATEX_FILE

FREE=$(grep -A 1 "ALL WITHOUT STRICT, DEPENDENT, AND EREPORT:" ../results/freestats.out | tail -n 1)
REALLOC=$(grep -A 1 "ALL WITHOUT STRICT, DEPENDENT, AND EREPORT:" ../results/reallocstats.out | tail -n 1)
echo "      C6: Arbitrary Functions& $FREE & $REALLOC \\\\
" >> $LATEX_FILE
